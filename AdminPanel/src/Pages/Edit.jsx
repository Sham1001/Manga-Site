...
const Edit = ({backendUrl}) => {
  ...
  const getManga = async()=>{
    try{
      ...
      params:{search, limit:4, page, admin:"admin"}
    })
    if(response.data.success){
   ...
   <div>
      ...
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-10 mr-10">
        ...
        <div className="w-full lg:w-1/2 space-y-8">
          <div className="grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-2 gap-6 m-auto">
            {
              allManga?.map((item)=>(
                <div
                  key={item._id}
                  className={`hover:shadow-[0_0_15px_5px_rgba(239,68,68,0.7)] rounded-2xl hover:scale-105 transition mr-10 border-2 ${mangaId === item._id ? 'border-red-500 shadow-[0_0_15px_5px_rgba(239,68,68,0.4)]' : 'border-transparent'}`}
                  onClick={()=>setMangaId(item._id)}
                >
                  <MangaComponent name={item.name} coverImg={item.coverImg} id={item.id} chapters={item.latestChapterNo} 
